/**
 * Single source of truth for the Alpine-branded email wrapper.
 * Each route just produces the body HTML and passes it through wrapEmail().
 *
 * Keeping the wrapper centralized prevents inconsistencies (mismatched header
 * styles, broken alt text, drift between transactional + marketing emails) and
 * makes branding changes a one-line edit.
 */

export interface WrapOptions {
  /** Override the default footer (e.g. add an unsubscribe link for newsletter). */
  footer?: string;
  /** Default: "ALPINE / Due Diligence". Pass a custom variant such as "ODD Admin". */
...
/** Plain-text → safe HTML (escape + preserve line breaks). */
