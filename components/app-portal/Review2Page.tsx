...
import { TOPIC_DATA as RIDGELINE_TOPIC_DATA, RIDGELINE_MOCK, VAULT_DATA as RIDGELINE_VAULT_DATA, FOLLOW_UP_MOCK as RIDGELINE_FOLLOW_UP_MOCK } from "@/lib/app-portal/ridgeline-data";
import { TRELLIS_TOPIC_DATA, TRELLIS_MOCK, TRELLIS_VAULT_DATA, TRELLIS_FOLLOW_UP_MOCK } from "@/lib/app-portal/trellis-data";
import { AURORA_TOPIC_DATA, AURORA_MOCK, AURORA_VAULT_DATA, AURORA_FOLLOW_UP_MOCK } from "@/lib/app-portal/aurora-data";
import { DocumentCollectionView } from "@/components/app-portal/review/FollowUpSection";
import { VerificationTab } from "@/components/app-portal/review/VerificationTab";
import { ReportWithMemo } from "@/components/app-portal/review/ReportWithMemo";
import { PlaceholderTab } from "@/components/app-portal/review/PlaceholderTab";
import OverviewSection from "@/components/app-portal/shell/OverviewSection";
import { TopicPage } from "@/components/app-portal/review/TopicPage";
import { formatAum } from "@/lib/app-portal/demo-utils";
...
function CallPrepTab() {
  ...
  useEffect(() => {
    if (!slug) return;
    fetch(`/api/app-portal/call-prep-notes?slug=${encodeURIComponent(slug)}`)
      .then((r) => r.json())
  ...
  async function saveNote(key: string) {
    ...
    try {
      await fetch("/api/app-portal/call-prep-notes", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
function AdminPortalTab({
  ...
  async function handleSend() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/app-portal/admin-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export default function Review2Page() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/app-portal/topic-rating?slug=${encodeURIComponent(slug)}`)
      ...
      .catch(() => {});
    fetch(`/api/app-portal/risk-obs?slug=${encodeURIComponent(slug)}`)
      .then((r) => r.json())
